...
LPC_GPIO_PORT->DIR[0] |= 0xFFFFFFFF;//make all pins ss Output
...
//	LPC_GPIO_PORT->PIN[0] ^= (1<<7);  //Toggle pin P0_7

	LPC_GPIO_PORT->PIN[0] ^= 0xFFFF;  //Toggle pin P0_7
//	bool state = false;
//	state = (state == false) ? true : false;
//	Chip_GPIO_SetPinState(LPC_GPIO_PORT, 0, 7, state);
//	Board_LED_Toggle(2);
...
int main(void) {
	...
	// PINENABLE0 affects only pins
	//PI0_0, PIO0_1, PIO0_3, PIO0_2, PIO0_8, PIO0_9, PIO0_5, PIO0_6
//	LPC_SWM->PINENABLE0 |= 0xFFFF;  //Enable default function (GPIO)
	Chip_Clock_EnablePeriphClock(SYSCTL_CLOCK_SWM);
	//Disable fixed pin functions thus enabling default function(GPIO)
	//bit 0 for p0_0 for d0
	//bit 4 for io_8 for d3
	//bit 5 for io_9 for d4
	//bit 8 for io_6 for d2
	LPC_SWM->PINENABLE0 |= (1 | (1<<4) | (1<<5) | (1<<8));
//	LPC_SWM->PINASSIGN[0] |= 0xFFFFFFFF; //Deselect any movable functions for pin0
//	LPC_SWM->PINASSIGN[4] |= 0xFFFFFFFF; //Deselect any movable functions for pin0
